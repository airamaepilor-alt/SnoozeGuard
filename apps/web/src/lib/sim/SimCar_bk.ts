...
export const EYE_LOCAL = new THREE.Vector3(0, 0.84, -0.35);
...
export class SimCar {
  ...
  private leftPillar!: THREE.Mesh;
  private rightPillar!: THREE.Mesh;
  private topFrame!: THREE.Mesh;
  private hood!: THREE.Mesh;
  private hoodRidge!: THREE.Mesh;
  private dashboard!: THREE.Mesh;
  ...
  private buildInterior() {
    ...
    this.dashboard = new THREE.Mesh(new THREE.BoxGeometry(1.72, 0.2, 0.5), dashMat);
    this.dashboard.position.set(0, 0.36, -1.12);
    this.interiorGroup.add(this.dashboard);
    ...
    this.leftPillar = new THREE.Mesh(
      new THREE.BoxGeometry(0.07, 0.72, 0.065),
      pillarMat,
    );
    this.leftPillar.position.set(-0.73, 0.66, -1.12);
    this.leftPillar.rotation.z = 0.18;  // lean outward
    this.leftPillar.rotation.y = 0.14;  // slight fore-aft tilt
    this.interiorGroup.add(this.leftPillar);

    this.rightPillar = new THREE.Mesh(
      new THREE.BoxGeometry(0.07, 0.72, 0.065),
      pillarMat,
    );
    this.rightPillar.position.set(0.73, 0.66, -1.12);
    this.rightPillar.rotation.z = -0.18;  // lean outward
    this.rightPillar.rotation.y = -0.14;  // slight fore-aft tilt
    this.interiorGroup.add(this.rightPillar);

    // Top windshield frame (connects tops of A-pillars)
    this.topFrame = new THREE.Mesh(
      new THREE.BoxGeometry(1.62, 0.07, 0.07),
      pillarMat,
    );
    this.topFrame.position.set(0, 0.845, -1.08);
    this.interiorGroup.add(this.topFrame);

    // ── Hood visible through lower windshield ────────────────────────────────
    this.hood = new THREE.Mesh(new THREE.BoxGeometry(1.88, 0.025, 0.56), bodyColorMat);
    this.hood.position.set(0, 0.285, -1.55);
    this.interiorGroup.add(this.hood);

    // Hood centre ridge
    this.hoodRidge = new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.04, 0.56), bodyColorMat);
    this.hoodRidge.position.set(0, 0.3, -1.55);
    this.interiorGroup.add(this.hoodRidge);
  ...
  hideViewObstructions(hidden: boolean) {
    this.leftPillar.visible = !hidden;
    this.rightPillar.visible = !hidden;
    this.topFrame.visible = !hidden;
    this.hood.visible = !hidden;
    this.hoodRidge.visible = !hidden;
    this.dashboard.visible = !hidden;
  }
